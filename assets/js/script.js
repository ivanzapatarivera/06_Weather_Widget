    ...
    // Calling for UV query URL 
        ...
        var queryURLuv = 'http://api.openweathermap.org/data/2.5/uvi?lat=' + latVal + '&lon=' + lonVal + '&appid=' + apiKey;
            ...
            $.ajax({
                ...
                var newsBrief = '<h3 class="pt-4 newsBrief textShadow">' + 'Forecast for ' + cityCountry + '</h3>' + '<p class="mt-5">' + main + ' are expected today with the possibility of ' + description + '.</p> <p>Today\'s temperature may rise to ' + maxConversion + '. Tonight, it may drop to ' + minConversion + '. </p> <p> However, the temperature may feel around ' + flConversion + ' to human touch, so plan accordingly for any outdoor activities. This is due to the level of humidity forecasted for today, which should be around ' + humidity + '. </p> <p>If you wish to plan ahead, please refer to the cards below with the weather outlook for the next five days.</p> <h5>Have a pleasant day and wish you a great week ahead!</h5>';
